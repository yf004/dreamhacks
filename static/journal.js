...
// Setup auto-save functionality (no visible indication)
function setupAutoSave() {
	// Auto-save every 30 seconds
	setInterval(() => {
		const journalDiv = document.getElementById('journal-text');
		localStorage.setItem('journalEntryHTML', journalDiv.innerHTML);
	}, 30000);
...
function saveJournal() {
	const journalDiv = document.getElementById('journal-text');
	const htmlContent = journalDiv.innerHTML;
	const textContent = journalDiv.innerText || journalDiv.textContent;
	
	// Save formatted HTML to localStorage
	localStorage.setItem('journalEntryHTML', htmlContent);
	
	// Create file download of HTML content to preserve formatting
	const htmlBlob = new Blob([
		...
		'  <style>\n' +
		'    body { font-family: Roboto Mono, monospace; padding: 20px; }\n' +
		'  </style>\n' +
		'</head>\n<body>\n' +
		htmlContent +
		'\n</body>\n</html>'
	...
	const date = new Date();
	const fileName = `journal_entry_${date.toISOString().split('T')[0]}.html`;
